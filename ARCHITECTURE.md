...
- `ui/screen/home/HomeScreen.kt`
  - Android 入口页面
  - 页面渲染与极少量页面级交互
  - 已提供 tracker 风格查询表单：`gray / anti / graynew / mode / genshin / pre / guid / components`

- `ui/screen/home/HomeViewModel.kt`
  - 查询执行入口
  - 表单状态、对话框状态、查询结果状态、历史记录状态与消息流
  - 收敛参数规范化、GUID 校验，以及 `gray / anti / graynew / genshin / pre / guid / components` 的历史恢复
  - 仅在区域层面对 `gray / graynew` 做 CN 可见性约束，不再把 `anti / gray / graynew` 收敛为互斥单选策略
...
- `HomeScreen.kt`
  ...
  - 当前已接入 `gray / anti / graynew / genshin / components / pre / guid`；但表单布局、说明文案与输入约束仍直接写在页面层，尚未继续拆为更小的表单组件。

- `HomeViewModel.kt`
  - 已收敛参数校验与高级参数历史恢复，但查询规则仍以内联条件表达，后续若继续扩大参数面，建议再抽离成独立表单规则层。
...
10. 已完成第七轮：Android UI 接入第一批查询参数 `gray / anti / graynew`。
11. 已完成第八轮：Android UI 接入第二批高级参数 `genshin / pre / guid / components`，并补齐 direct taste / preview 的 GUID 约束与历史恢复。
12. 已完成第九轮：重新对照 tracker 语义后，取消了把 `gray / anti / graynew` 折叠成互斥单选策略的做法，也取消了 `anti -> taste` 与 `graynew -> stable` 的表单层强锁定。
13. 待完成：继续把更多 UI 事件与数据加载边界从 `HomeScreen` 收敛到更清晰的状态模型中，再评估是否继续扩大 `ViewModel` 职责。
