...
- 仍缺少高级查询能力
  ...
  - 已完成首版 `anti` 前缀策略与 CN `gray` host 切换，`graynew / genshin` 仍未迁移
...
4. 评估未来新增的 `components / graynew / genshin` 等能力是否需要继续进入 CLI
...
7. 待完成：继续把 `graynew / genshin / components` 等差异能力逐步迁入，同时控制 `QueryUpdateArgs` 的复杂度。
