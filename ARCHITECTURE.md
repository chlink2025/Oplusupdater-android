...
- `HomeScreen.kt`
  - 已通过 `HomeViewModel` 拆走表单状态、查询执行、历史记录与消息流，但仍直接协调部分页面事件与组合细节。
...
6. 待完成：继续把更多 UI 事件与数据加载边界从 `HomeScreen` 收敛到更清晰的状态模型中，再评估是否继续扩大 `ViewModel` 职责。
