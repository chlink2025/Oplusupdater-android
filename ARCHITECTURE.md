...
- `HomeViewModel.kt`
  - 已收敛策略联动、GUID 校验与高级参数历史恢复，但查询规则仍以内联条件表达，后续若继续扩大参数面，建议再抽离成独立表单规则层。
...
12. 待完成：继续把更多 UI 事件与数据加载边界从 `HomeScreen` 收敛到更清晰的状态模型中，再评估是否继续扩大 `ViewModel` 职责。
